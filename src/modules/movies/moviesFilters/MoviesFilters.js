...
import { UseDispatch, shallowEqual, useDispatch, useSelector } from 'react-redux';
...
const SearchModal = ({ isOpen, setIsOpen }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      console.log(values);
      dispatch(searchMovie(`/search/movie?query=${values?.title}&include_adult=${values?.isAdult}&language=${values?.language}&page=1'`))
       setIsOpen(false); },
  });
  ...
  return (
    ...
    <IoIosSearch  onClick={() => setIsOpen(prev => !prev)}
        className=" w-10 h-10 bg-white cursor-pointer " title='Search'
    />
    </div>
    
      
      
      <Dialog open={isOpen} onClose={() => setIsOpen(prev => !prev)} className="relative z-10" >
        {/* The overlay */}
